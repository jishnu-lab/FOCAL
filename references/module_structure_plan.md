# Focal module structure — proposal (moscot-inspired)
...
Focal is three modules in one package:
...
│
├── state_specific/              # ========= FocalStateSpecific =========
│   ├── __init__.py
...
│
├── cross_prediction/            # ======== FocalCrossPrediction ========
│   ├── __init__.py
...
| `analysis/ensure_focal_path.py` | **deleted** | replaced by `pip install -e .` |
...
### 2b. `src/focal/` internal re-shuffle
...
```python
import focal as ff
from focal.temporal import EpisodeDynamics, SmoothedCurvesGRN, TFForceWaves
...
replaced by `importlib.reload(focal.temporal._waves)` on the real module.
...
- `bash_scripts/{network/1_network_reconstruct,preproc/0_get_expression}.sbatch` — currently point
  ...
  `python -m focal.cli.reconstruct_networks`.
...
* `bash_scripts/{network/1_network_reconstruct,preproc/0_get_expression}.sbatch`: the dead
  `/ocean/...` script paths became `python -m focal.cli.*`.
...
* **One import instead of two.** `from focal.temporal import *` now yields the
