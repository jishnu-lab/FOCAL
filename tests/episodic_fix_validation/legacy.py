...
Two defects were fixed in ``src/focal/temporal/`` (both `CONFIRMED` in
...
Nothing here is importable from ``focal``; it is frozen dead code by design.
"""
...
from focal.utils.parallel import create_balanced_chunks


def legacy_calculate_force_curves_chunk(
    beta_chunk: pd.DataFrame, tf_expression: pd.DataFrame, epsilon: float = 1e-10
) -> pd.DataFrame:
    """Pre-fix ``focal.temporal._forces.calculate_force_curves_chunk`` (ISSUES #1).
