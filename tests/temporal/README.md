# Tests for `focal.temporal`

Coverage of `src/focal/temporal/_episodes.py` and
`src/focal/temporal/_curves.py`: every public class and function, the
...
| `test_episodic_units.py` | module-level helpers, now spread across three modules: the ORA primitive (`focal.base`), edge filtering and force curves (`temporal/_forces.py`), chunking (`focal.utils`), episode subsetting |
